...
const SoftballProfile = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [profileRes, skillsRes, recsRes] = await Promise.all([
        supabase.from("profiles").select("*").eq("user_id", session.user.id).single(),
        supabase.from("skill_progression").select("*").eq("user_id", session.user.id).eq("sport_type", "softball"),
        supabase.from("development_recommendations").select("*").eq("athlete_user_id", session.user.id).eq("sport_type", "softball").eq("status", "pending").order("created_at", { ascending: false }).limit(5),
      ]);
